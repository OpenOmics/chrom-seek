...
class TestParsePeakCall(unittest.TestCase):
    ...
    def test_bad_group_labels(self):
        ...
        with open(path, "w") as f:
            f.write(str2file)
            f.close()
        with self.assertRaisesRegex(ValueError, r"Group\(s\)\: g-2, g\*3"):
            chip2input, groups, blocks = peakcalls(path)

    def test_group_labels_allow_underscores(self):
        str2file = (
            "Sample\tGroup\n"
            + "one\tctrl_rep_1\n"
            + "two\tctrl_rep_2\n"
        ...
        with open(path, "w") as f:
            ...
            f.close()
        chip2input, groups, blocks = peakcalls(path)
        self.assertIn("ctrl_rep_1", groups)
        self.assertIn("ctrl_rep_2", groups)
    ...
    def test_group_labels_reject_reserved_vs_token(self):
        ...
        with open(path, "w") as f:
            f.write(str2file)
            f.close()
        with self.assertRaisesRegex(ValueError, r"reserved token _vs_"):
            peakcalls(path)
    ...
    def test_bad_headers(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"Peakcall file has unsupported headers!"
        ):
            chip2input, groups, blocks = peakcalls(path)
    ...
    def test_atac_with_input_control(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"ATAC-seq assay does not support InputControls, please leave InputColumn empty",
        ):
    ...
    def test_input_control_equals_sample_positive(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"InputControl cannot be the same as Sample. All these samples have an error: Sample1",
        ):
            chip2input, groups, blocks = peakcalls(path)
    ...
    def test_block_equals_sample_positive(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"Block cannot be the same as Sample. All these samples have an error: Sample1",
        ):
            chip2input, groups, blocks = peakcalls(path)
    ...
    def test_group_equals_sample_positive(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"Group cannot be the same as Sample. All these samples have an error: Sample1",
        ):
            chip2input, groups, blocks = peakcalls(path)
    ...
    def test_multiple_validation_errors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"InputControl cannot be the same as Sample"
        ):
            chip2input, groups, blocks = peakcalls(path)
