...
from .files import validate_custom_genome
...
def init(
    ...
    output_path,
    links=[],
    required=["workflow", "bin", "resources", "config"],
):
...
def setup(sub_args, ifiles, repo_path, output_path):
    ...
    if sub_args.genome.endswith(".json"):
        validate_custom_genome(sub_args.genome)
    ...
    config["project"]["binpath"] = os.path.abspath(
        os.path.join(config["project"]["workpath"], "bin")
    )
