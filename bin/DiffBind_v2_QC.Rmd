...
PCAbasic <- function(pv, max_comps = 3) {
  # Note 1: The functions calculating PCA come directly from the code within
  # DiffBind 2.15.2. 
  # Note 2: PCA is on a log2 scale when counts are available.
...
```{r samples}
# Build from the preflight-filtered sheet (not the raw CSV path) so rows dropped
# for NaN/empty peaks or BAMs never reach DiffBind.
samples <- dba(sampleSheet=sample_sheet)
