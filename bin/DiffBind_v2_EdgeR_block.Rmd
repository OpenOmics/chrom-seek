...
```{r samples}
full_list <- "_Diffbind_EdgeRblock_fullList.bed"
...
```{r heatmap4D}
try(dba.plotHeatmap(DBAnalysisEdgeR,contrast=1,method = DBA_EDGER_BLOCK,correlations=FALSE,margin=20,cexRow=1,cexCol=1),silent=TRUE)
```
...
write.table(report2, full_list, quote=F, sep="\t", row.names=F)
```
